...
public class SectionService(ApplicationDbContext context,
							...
							ILogger<SectionService> logger) : ISectionService
{
	...
	public async Task<Result<IEnumerable<SectionResponse>>> GetAllByCourseAsync(string courseKeyId, CancellationToken cancellationToken = default)
	{
	...
	public async Task<Result<SectionResponse>> CreateAsync(string courseKeyId, SectionRequest request, string userId, CancellationToken cancellationToken = default)
	{
		//var courseIds = Decode(courseKeyId);
		...
		var exists = await _context.Sections.AnyAsync(s => s.CourseId == courseId && s.Position == request.Position && !s.IsDeleted, cancellationToken);
		if (exists)
	...
	public async Task<Result<SectionResponse>> UpdateAsync(int id, SectionUpdateRequest request, string userId, CancellationToken cancellationToken = default)
	{
		...
		var conflict = await _context.Sections
			.AnyAsync(s => s.CourseId == section.CourseId && s.Id != section.Id && s.Position == request.Position && !s.IsDeleted, cancellationToken);
		if (conflict)
			return Result.Failure<SectionResponse>(SectionErrors.SectionPositionConflict);
