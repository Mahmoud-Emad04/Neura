...
[ApiController]
[Authorize]
public class AnnouncementController(IAnnouncementService announcementService, ILogger<AnnouncementController> logger) : ControllerBase
{
	...
	[HttpGet("posts")]
	[AllowAnonymous]
	public async Task<IActionResult> GetAllPosts([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10, CancellationToken cancellationToken = default)
	{
	...
	public async Task<IActionResult> UpdatePost([FromRoute] int postId, [FromBody] PostUpdateRequest request, CancellationToken cancellationToken = default)
	{
		var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
	...
	public async Task<IActionResult> TogglePostVisibility([FromRoute] int postId, CancellationToken cancellationToken = default)
	{
		var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
	...
	public async Task<IActionResult> UpdatePostComment([FromRoute] int commentId, [FromBody] PostCommentUpdateRequest request, CancellationToken cancellationToken = default)
	{
		var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
	...
	public async Task<IActionResult> RemovePostComment([FromRoute] int commentId, CancellationToken cancellationToken = default)
	{
		var userId = User.FindFirstValue(ClaimTypes.NameIdentifier)!;
